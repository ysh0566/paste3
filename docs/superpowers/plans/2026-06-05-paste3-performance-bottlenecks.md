...
- [x] **Step 3: 添加非功能测试确保 probe 不改变返回值**
...
- [x] **Step 1: 给高频查询字段加 SwiftData 索引**
...
- [x] **Step 2: 移除 ClipboardPayloadStore 的 MainActor 约束**
...
- [x] **Step 5: 修改 ClipboardMonitor 异步处理**
...
自动验证覆盖：`classifierLimitsLargeTextAndSearchTextWork` 验证 200k 文本截断和搜索文本上限，`classifierBuildsLargePayloadSnapshotCandidate` 验证 10MB payload 快照候选构建，`searchIndexFindsTargetInLargeHistoryCandidateSet` 验证 5k 历史索引预筛选，`storePrunesExpiredItemsInBatches` 验证 600+ 过期记录分批清理。
