...
- 图片、富文本、HTML 和通用数据 payload 会保存到本机 Application Support 文件目录，SwiftData 只保留可检索元数据。
- 按内容 hash 去重，同一内容从不同来源复制会分别保留。
- 支持普通搜索，以及 `app:`、`type:`、`from:`、`pin:` 查询过滤。
- 支持暂停捕获、排除指定 App、删除单条记录、清空历史。
- 菜单栏入口和底部 quick panel。
- 支持复制回剪贴板；授权辅助功能权限后可自动粘贴回前台应用。
- 默认保留最近 1 年历史，并可在设置中切换为 1-6 天、1-3 周、1-11 个月、1-3 年或永久保留。
...
- 可在设置中暂停捕获或排除指定 Bundle ID，默认排除 1Password、Keychain Access 和 Passwords 等敏感 App。
- 辅助功能权限只用于把选中的历史项自动粘贴回原前台应用。
- 用户可以从菜单中清空历史记录。

## 搜索语法

搜索框支持普通关键词，也支持几个确定性过滤器：

- `app:Terminal`：按来源 App 名称或 Bundle ID 过滤。
- `type:command`：按内容类型过滤，支持 `text`、`link`、`command`、`image`、`file`、`rich`、`data`。
- `from:today`：按时间过滤，支持 `today`、`yesterday`、`week`、`month`、`year` 和 `7d` 这类相对天数。
- `pin:PR`：按 Pinboard 名称过滤，`pin:any` 表示任意已 pin 项目。
